...
- Локальный VAD из исходного Sales Helper: цельная реплика определяется по паузе и отправляется напрямую в Gemini блоками 16 kHz PCM.
- Подсказка обновляется в компактном overlay по мере поступления фрагментов Gemini.
- Только компактный overlay без окна истории; поле подсказки рассчитано на пять строк.
